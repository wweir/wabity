...
impl StreamingPayloadCollector {
    ...
    fn push_bytes_with_text_stream<F>(&mut self, chunk: &[u8], on_text_delta: &mut F) -> Result<()>
    where
        F: FnMut(&str),
    {
...
impl ResponsesSseState {
    ...
    fn apply_event<F>(&mut self, payload: &Value, on_text_delta: Option<&mut F>) -> Result<()>
    where
        F: FnMut(&str),
    {
        self.saw_event = true;
...
impl ChatCompletionsSseState {
    fn apply_chunk<F>(&mut self, payload: &Value, mut on_text_delta: Option<&mut F>) -> Result<()>
    where
        F: FnMut(&str),
    {
        self.saw_chunk = true;
