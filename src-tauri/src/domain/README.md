...
- Agent 内置工具模块的状态必须显式建模，避免前端把旧的“固定 URL + 运行状态 + 最近错误”继续当成对外 server 能力
...
- Agent MCP 配置必须能稳定表达 MCP 服务清单，至少覆盖 `stdio/http/sse` 三类 transport，以及 `args`、`env`、`headers` 这些连接参数；Wabity 不再把它解释成对外暴露的本地 server
