...
- 定义 workspace、Pi Agent session 等前后端通信结构
- 定义历史剪贴板快照和条目模型
- 定义文件搜索结果等前后端通信结构
- 定义已安装应用搜索结果等前后端通信结构
...
- Pi Agent session 状态必须显式区分摘要信息和消息流详情；摘要必须能区分可恢复错误和不可恢复错误
...
- Pi Agent 配置只表达内嵌 Pi SDK session 默认值，例如 provider、model、thinking、enabled tools 和最大工具迭代次数；不再表达外部 agent 命令、启动模式或默认 ACP agent 目录
