...
- `pi_agent` 不再支持 ACP `stdio` transport，不声明 ACP client capability，也不透传 `mcp_servers`；Agent session 只固定传入 workspace，并会优先把 AI 功能页里的文档问答模型桥接到 Pi SDK `SessionOptions`。当前只自动桥接 Pi SDK 已知 provider（OpenAI / OpenRouter / DeepSeek / Ollama / SiliconFlow）；其它自定义 OpenAI-compatible endpoint 不强行映射，继续使用 Pi SDK 自身配置兜底
