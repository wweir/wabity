...
- `ThoughtDisclosure.tsx`：提取 launcher 内通用的 thought 折叠展示；Pi Agent 时间线和翻译结果都复用同一套预览、展开语义和阅读排版，避免同一类次级信息出现两套行为
- `SessionTimeline.tsx`：渲染 Pi Agent 会话消息流与 launcher 轻量问答历史；assistant message 必须按后端提供的 block 原始顺序渲染，保留 `thought / actions / content` 的真实交错时序，不能再为了“摘要化”重排成固定答案优先结构；正文块仍保持主阅读排版，action trail 和 thought 继续作为次级信息，但只能靠视觉权重降低，而不能靠改写顺序；thought 默认折叠，只提供简短预览，展开内容使用普通阅读排版而不是日志式 `pre`；Pi Agent 流式输出时只在用户仍跟随当前 turn 尾部时自动追随新增文本，用户手动滚离后不得抢回滚动位置
