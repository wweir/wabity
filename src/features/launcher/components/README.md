...
- `LauncherComposer.tsx`：渲染主输入区和底部操作按钮
...
- `SessionTimeline.tsx`：渲染 ACP 会话消息流、thought 折叠和 action pill 详情，并复用共享 MarkdownRenderer
...
- 输入、选择和关闭等行为都通过 props 回调回传给 `LauncherPage`
