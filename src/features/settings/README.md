...
- 设置页整体交互改成左侧常驻导航：左侧只负责分组切换，当前分组的 quick jump 合并进主内容区标题行，避免再拆一层独立 jump rail 抬高首屏
- 设置页的滚动容器固定在右侧主内容区；左侧分组导航不跟着右侧配置滚动，避免长表单场景里导航位置漂移
- 左侧导航列保持窄宽度，优先把可用宽度让给右侧编辑区；右侧配置卡、说明卡和长命令文本必须允许收缩换行，不能让设置页出现横向滚动条
- 主内容区顶部只保留一行 section header：分组标题、简短说明和 quick jump 共用同一块 header，在窄窗口下再回退换行
...
- LLM 页在“还没有条目”时先退成单列 onboarding，不提前展示完整双栏编辑器；只有存在条目后才进入目录 + 编辑工作台
...
- AI 功能页按“翻译配置 / 文档问答配置”两个独立任务卡组织；宽窗口下优先双列并排，避免两张长卡把首屏压成纵向长卷
