...
- 保存模型接入分组时，前端仍只提交“当前 provider/model 草稿 + 已落盘的翻译 / 问答 / OCR / RAG 引用”；真正的依赖引用修复必须由 Rust 侧基于最新 LLM 目录先归一化，再做跨分组校验，避免删除或替换模型后因为旧引用残留而误报保存失败
...
- LLM 页的编辑节奏对齐 `AI 功能` 和 `RAG`：状态条之后收敛成“供应商预设与连接 / 模型、调用方式与用途”两段主流程；页面整体保持单列，但条目目录固定用双栏卡片提高扫读效率，普通短字段统一用“左侧字段名 / 右侧控件与帮助”，字段名列缩到最小必要宽度，输入区默认吃满剩余空间，同时主编辑区与分段卡保留稳定横向内边距，避免表单内容直接贴边；模型来源、模型选择器、长说明和能力说明继续占满整行
...
- LLM 模型名优先是普通手填输入框；远端目录只是辅助候选弹层，使用普通按钮列表承接键盘遍历和点击填入，不伪装成未完整实现的 `combobox + listbox + option` 组合。无论从输入框箭头还是右侧按钮打开，焦点都必须进入当前候选或首项，关闭或选中后再回到输入控件或触发按钮
