...
- 通过一级导航把通用、AI 功能、LLM、RAG、ACP Agent、MCP、Skill、关于拆成独立设置分组，而不是单个长表单；快捷键、通知、外观和 OCR 配置并入通用页
...
- 配置卡片内部的字段列表允许按内容选择双列：RAG、LLM、ACP、MCP 这类编辑卡里的多个短字段可以左右分布，header、动作区、`textarea`、模型选择器、多行目录和长说明块自动跨满整行；AI 功能任务卡保持单列任务流，不参与这套共享双列
...
- LLM 页面条目选择区使用卡片网格，但语义上必须是单选目录：目录容器暴露 `radiogroup`，条目卡暴露 `radio`，键盘方向键可在条目间切换，不能继续拿普通按钮 + `aria-pressed` 冒充互斥选择
- LLM 条目卡片只负责展示条目本身和当前用途标签；翻译 / 问答模型选择统一移到 AI 功能页，LLM 页不再承载“默认 LLM”逻辑
- 单个 LLM 条目的编辑区改成“先选配置类型，再填基础信息和模型名”；配置类型把 `modelType + protocol + supportsStateful` 合并成单个选择器，直接区分 `LLM · responses stateless`、`LLM · responses stateful`、`LLM · chat/completions` 和 `Embedding` 四种页面。入口只保留一条紧凑状态/操作条，不再叠独立草稿卡或 hero 摘要；用途说明只保留必要能力约束，避免把目录里已经可见的身份信息再重复一遍
...
- 默认快捷键当前为 launcher=`Alt+Space`、翻译选中文本，未选中时 OCR=`Alt+D`
- LLM 条目当前支持多条目录项；`Base URL` 由用户显式填写到 API 根路径，通常包含 `/v1`，`API key` 字段会在模型选择前展示，便于先完成连接信息
- LLM 页在“还没有条目”时先退成单列 onboarding，不提前展示完整编辑工作台；有条目后也保持“目录在前、编辑在后”的单列流，避免在窄窗口里再切一层左右分栏
- LLM 页的主编辑入口优先让位给真实字段：目录之后直接进入紧凑状态/操作条和分段编辑卡，不再让 Base URL、模型名、配置类型这些已在目录卡里出现过的信息继续占据首屏摘要位，也不允许退回成一整条细长表单
- LLM 页把“使用模板”“配置类型”和“模型来源”视为显式模式切换，而不是普通填空字段：模板负责填供应商默认值和白名单边界，配置类型负责改写协议与能力，模型来源区则明确区分白名单、远端目录和手填
- LLM 页的编辑节奏对齐 `AI 功能` 和 `RAG`：状态条之后按“接入模式 / 基础连接 / 模型 / 用途与能力”分成多段卡片，字段标签默认上下堆叠，优先保证输入宽度和扫读节奏
- LLM 条目统一先通过“新增”进入上方目录，再在后面的编辑区顶部用“使用模板”下拉决定是否套用内置供应商模板；套用后自动填入 `Base URL`、协议、能力标签和白名单模型
...
- LLM 模型选择器虽然允许手填，但语义上必须是完整的 `combobox + listbox + option` 关系，并支持键盘打开、关闭和候选遍历；不能只做“一个输入框旁边挂个按钮”
...
- AI 功能任务卡不再外包一层 detail 卡；每张卡只保留单层编辑容器，顶部只放标题、状态和一句说明，动作统一下沉到底部按钮区
