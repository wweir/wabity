...
- 2026-04-25：模型接入页面继续降噪，宽窗口改为 provider 目录 / 当前编辑双栏，字段按钮移出 `label`，模型候选弹层改为可手填 combobox + listbox 语义
- 2026-04-25：模型接入页面继续收紧顶部留白；有条目时目录和编辑区已经首屏并列展示，不再显示标题下 quick jump，页头之后直接进入双栏工作区
- 2026-04-25：修正 LLM 右侧编辑区盒模型；外层卡片恢复稳定内边距，状态条和分段标题不再贴边或越框
- 2026-04-28：模型候选面板改为字段内占位并限制列表自身滚动，避免远端模型目录较长时被设置窗口或编辑卡片裁切
- 2026-04-28：前端初始化 LLM 草稿时结合内置模板目录清理无效 `builtinPresetModelId`，Rust 保存链路在校验前同步归一化并记录保存失败错误，避免旧草稿导致“模型接入配置保存失败”只显示通用提示
- 2026-04-28：组内模型切换从名称按钮墙改成纵向单选目录，每行展示模型名、调用方式和可用功能，并支持方向键切换
